...
def test_multiple_select_field_type_rows(data_fixture, django_assert_num_queries):
    ...
    with pytest.raises(AllProvidedMultipleSelectValuesMustBeIntegersOrStrings):
        row_handler.create_row(
            user=user,
            table=table,
    ...
    row_1 = row_handler.create_row(
        user=user, table=table, values={f"field_{field.id}": ["Option 2"]}
    )
    ...
    assert getattr(row_5_field[1], "id") == select_options[1].id
