...
from baserow.core.jobs.constants import JOB_FINISHED, JOB_PENDING
...
def test_can_submit_duplicate_application_job(data_fixture):
    ...
    duplicate_application_job = JobHandler().create_and_start_job(
        ...
        user_websocket_id=web_socket_id,
    )

    assert Application.objects.count() == 1

    assert duplicate_application_job.state == JOB_PENDING
    assert duplicate_application_job.original_application_id == application.id
    assert duplicate_application_job.duplicated_application is None

    run_async_job(duplicate_application_job.id)
