...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"{database_count} tables have been created.")
        )
...
def fill_group_with_data(
    ...
    min_field_count = max(1, int(field_count * (1 - percentage_variation / 100)))
    max_field_count = int(field_count * (1 + percentage_variation / 100))
    min_row_count = max(1, int(row_count * (1 - percentage_variation / 100)))
    max_row_count = int(row_count * (1 + percentage_variation / 100))
        ...
        for _ in range(database_count):
            with transaction.atomic():
                ...
                for _ in range(table_count):
                    ...
                    table_fields_count = randint(min_field_count, max_field_count)
                    print(
                        f"Creating {table_fields_count} fields for table {table.id} from process {process_id}"
                    )
                    fill_table_fields(field_count, table)
                    table_rows_count = randint(min_row_count, max_row_count)
                    print(
                        f"Creating {table_rows_count} rows for table {table.id} from process {process_id}"
                    )
                    fill_table_rows(table_rows_count, table)
                    pbar.update(1)
