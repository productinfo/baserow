<template>
    <div>
      <input
        ...
        v-model="dateString"
        type="text"
        class="input crud-table-filters__value-input"
        :disabled="disabled"
      ...
      >
        <client-only>
          <date-picker
            ...
            class="datepicker"
            @input=";[setCopy($event, 'dateObject'), $emit('input', copy), $refs.dateContext.hide()]"
          ></date-picker>
        </client-only>
      </Context>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getDatePlaceholder(field) {
        return this.$t(
          'humanDateFormat.' + getDateHumanReadableFormat('US')
        )
      },
      focus() {
